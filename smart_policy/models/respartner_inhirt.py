...
class inhertResPartner(models.Model):
    ...
    opps_count = fields.Integer(string=" Opps Count", required=False, compute='_compute_opps_count')

    @api.one
    @api.depends()
    def _compute_opps_count(self):
        self.opps_count = self.env['crm.lead'].search_count([('partner_id', '=', self.id)])

    @api.multi
    def filter_claim(self):
        return {
