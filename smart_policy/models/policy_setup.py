...
class Brokerage(models.Model):
    ...
    collection_bonus = fields.Float(' Collection Bonus')
    ...
    product_id = fields.Many2many('insurance.product',
                                  domain="[('insurer','=',insurer),('line_of_bus','=',line_of_business)]")
    line_of_business = fields.Many2one('insurance.line.business', string='Line of Business')
    insurer = fields.Many2one('res.partner', string="Insurer", required=True, domain="[('insurer_type','=',1)]")
    commission_rule = fields.Selection([('basic', 'Basic'),
                                        ('collection', 'Collection Bonus'),
                                        ('production', 'Production Commission'), ], string='Commission Rule',
                                       track_visibility='onchange', required=True)
    pro_commission_data = fields.One2many('pro_com_data','inverse', string='Production Commission')
...
class Data(models.Model):
    ...
    inverse = fields.Many2one('product.brokerage')
